...
- Fixed automatic 3D modeswitch not working reliably, needed on my AM6B+ and my NVidia Shield but cannot be reproduced by CE maintainer
...
- Fixed BD JRE menu mode bugs, all tested BD/3DBD/UHD ISOs work reliably including JRE pop-up menus
- Changed HDMI modeswitch behaviour in (3D)BD menu mode. If a 3D BD is recognized, a modeswitch to the desired 3D mode is triggered only once at playback start. After that, all content is displayed in this 3D mode until playback stop, regardless of whether it's 2D or 3D content.
